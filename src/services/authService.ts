...
/**
 * Tuỳ chọn: nếu dự án có StorageManager (local cache)
 * thì dòng declare này giúp TypeScript không báo lỗi khi không import.
 */
...
function saveUserToStorage(u: {
  ...
  try {
    ...
    localStorage.setItem("emyland_user", JSON.stringify(cur));
  }
  return cur;
}
...
function deviceId() {
  ...
  if (!v) {
    v = `${navigator.userAgent.slice(0, 32)}-${Math.random()
      .toString(36)
      .slice(2)}`;
...
export async function login(emailOrPhone: string, password: string) {
  const { data, error } = await supabase.rpc("rpc_login", {
    // ⚠️ PHẢI khớp tên tham số function: (p_email_or_phone text, p_password text, p_device_id text)
    p_email_or_phone: emailOrPhone,
    p_password: password,
    p_device_id: deviceId(),
    // p_ttl_minutes: 43200, // 30 ngày (giữ mặc định của server nếu không truyền)
  });
...
export async function logout() {
  const t = getToken();
  try {
    if (t) await supabase.rpc("rpc_logout", { p_token: t });
  } catch {
    /* noop */
  }
  clearToken();
  try {
    (StorageManager as any)?.logout?.();
  } catch {
    /* noop */
  }
}
