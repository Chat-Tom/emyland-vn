    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k.includes("legal") && k.includes(id)) {
        ...
        try {
          ...
          if (Array.isArray(arr)) out.push(...arr.filter(Boolean)); // ✅ spread đúng
        } catch {}
      }
    }
  } catch {}

  // ✅ khử trùng lặp & chỉ giữ giá trị truthy
...
const PostProperty: React.FC = () => {
  ...
  const loadPropertyForEdit = async (id: string) => {
    ...
    const __lsLegal = getLegalImagesByIdLocal(id);
    const legalImages = (__lsLegal && __lsLegal.length
      ? __lsLegal
  ...
  const onSubmit = async () => {
    ...
    if (isEditMode && editId) {
      ...
      const rowUpdate = {
        ...
        legal_images: JSON.stringify(form.legalImages || []),
