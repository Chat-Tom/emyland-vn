...
/* ✅ THÊM: login Supabase gốc (email + password) để có session thật cho AuthSync */
async function trySupabasePasswordLogin(loginId: string, pwd: string) {
  if (!isEmail(loginId)) return false; // Supabase password login chỉ áp dụng cho email
  try {
    ...
    // Lưu access_token để tái dùng cho rpc_me (nếu cần)
...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      /* ✅ SỬA NHẸ: Ưu tiên Supabase email/password nếu nhập email; nếu không thì RPC cũ */
      let cloudOK = false;
      if (isEmail(loginId)) {
